...
class WPmediaburstSMS {
	...
	public function __construct( $username="", $password="", $options = array() ) {
		if( empty($username) && !defined('MEDIABURST_USER') ) 
			throw new WPmediaburstException("Username is blank");
		else
			$this->username = (empty($username)) ? MEDIABURST_USER : $username;
		if( empty($password) && !defined('MEDIABURST_PASS') )
			throw new WPmediaburstException("Password is blank"); 
		else
		...
		$this->http_class = (array_key_exists('http_class', $options)) ? $options['http_class'] : 'WPmediaburstHTTP';
	...
	public function Send( $to, $message ) {
		...
		if(isset($err_no)) {
			throw new WPmediaburstException($err_desc, $err_no);
		}
 ...
 * @since	1.0
 */
class WPmediaburstException extends Exception {
